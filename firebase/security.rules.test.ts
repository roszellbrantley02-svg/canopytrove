...
describe('Firestore users rules', () => {
  it('denies customers from writing an owner role into their own user document', async () => {
    const db = testEnv.authenticatedContext('customer-1', {
      role: 'customer',
    }).firestore();
...
describe('Firestore storefront_review_photos rules', () => {
  ...
  it('denies other signed-in users from reading pending review-media records', async () => {
    ...
    const db = testEnv.authenticatedContext('customer-2', {
      role: 'customer',
    }).firestore();
    ...
    await assertSucceeds(
      setDoc(
        doc(db, 'storefront_review_photos', 'media-4'),
        {
          ...
          storefrontId: 'disp-1',
          reviewId: 'review-1',
          storagePath: 'community-review-media/pending/customer-1/disp-1/review-draft-1/media-4.jpg',
          approvedStoragePath: 'community-review-media/approved/disp-1/review-1/media-4/media-4.jpg',
...
describe('Storage owner media rules', () => {
  it('denies signed-in customers on the legacy deals-media path', async () => {
    const storage = testEnv.authenticatedContext('customer-1', {
      role: 'customer',
    }).storage();
  ...
  it('denies anonymous users from writing pending community review media', async () => {
    ...
    await assertFails(
      uploadString(
        ref(
          storage,
          'community-review-media/pending/customer-1/disp-1/review-draft-1/photo-1.jpg'
        ),
        'anonymous upload',
        'raw',
        {
  ...
  it('denies other users from writing into someone else pending community review media path', async () => {
    ...
    await assertFails(
      uploadString(
        ref(
          storage,
          'community-review-media/pending/customer-2/disp-1/review-draft-1/photo-1.jpg'
        ),
        'wrong uploader',
        'raw',
        {
  ...
  it('allows admins to publish approved community review media and blocks direct public reads', async () => {
    const storage = testEnv.authenticatedContext('admin-1', {
      admin: true,
      role: 'admin',
    }).storage();
  ...
  it('denies signed-in customers on the approved owner-media path even with a matching uid', async () => {
    ...
    await assertFails(
      uploadString(
        ref(storage, 'dispensary-media/disp-1/approved/owner/customer-1/storefront-card/customer-banner.txt'),
        'customer upload'
      )
    );
  });
  ...
  it('denies owners without canonical verification and subscription state', async () => {
    ...
    await assertFails(
      uploadString(
        ref(storage, 'dispensary-media/disp-1/approved/owner/owner-1/storefront-card/owner-banner.txt'),
        'owner upload'
      )
    );
  });
  ...
  it('denies owners from writing media into another owner uid path', async () => {
    ...
    await assertFails(
      uploadString(
        ref(storage, 'dispensary-media/disp-1/approved/owner/owner-2/storefront-card/owner-banner.txt'),
        'owner upload'
      )
    );
  });
  ...
  it('denies owners without an active subscription on the approved owner-media path', async () => {
    ...
    await assertFails(
      uploadString(
        ref(storage, 'dispensary-media/disp-1/approved/owner/owner-1/storefront-gallery/owner-banner.txt'),
        'owner upload'
      )
    );
  });
  ...
  it('allows admins on both deals-media path shapes', async () => {
    ...
    await assertSucceeds(
      uploadString(
        ref(storage, 'deals-media/disp-1/deal-1/admin-banner.txt'),
        'admin upload'
      )
    );
    await assertSucceeds(
    ...
    await assertSucceeds(
      uploadString(
        ref(storage, 'dispensary-media/disp-1/approved/owner/owner-1/storefront-card/admin-owner-banner.txt'),
        'admin upload',
        'raw',
        {
          contentType: 'image/png',
  ...
  it('blocks direct public and customer reads on both deals-media path shapes', async () => {
    const adminStorage = testEnv.authenticatedContext('admin-1', {
      admin: true,
      role: 'admin',
    }).storage();
    ...
    await assertSucceeds(
      uploadString(
        ref(adminStorage, 'deals-media/disp-1/deal-1/admin-banner.txt'),
        'admin upload'
      )
    );
    await assertSucceeds(
    ...
    const customerStorage = testEnv.authenticatedContext('customer-1', {
      role: 'customer',
    }).storage();
