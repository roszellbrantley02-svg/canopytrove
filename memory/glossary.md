...
| OCM cache        | Hourly snapshot of the NY OCM public license dataset used for consumer verification                                                         | ocmLicenseCacheService.ts                  |
| OCM verification | `ocmVerification` field on every storefront summary/detail — drives the "Verified licensed" badge                                           | storefrontOcmEnrichment.ts                 |
| verify tab       | Camera-first tab where users scan QR codes/barcodes (products or shops) or manually enter license/name/address                              | VerifyScreen.tsx                           |
| COA              | Certificate of Analysis; state-lab report showing cannabinoid potency, terpenes, and contaminant test results for a specific cannabis batch | Mandatory on every legal NY product        |
| COA URL          | Public URL (usually QR code on product package) linking to the lab's hosted PDF/web report for that batch                                   | Lab-issued, NY-licensed                    |
| product scan     | User-initiated camera capture in Verify tab resolving to either a shop (OCM license match) or a product (COA match)                         | Anonymous install-level logging            |
| brand counter    | Aggregated Firestore document counting scans per brand across regions and time, powering trending and operator analytics                    | brandCounters collection                   |
| scan resolution  | Discriminated-union result of the backend scan router: `license`, `product`, or `unknown`                                                   | scanIngestionService.ts                    |
...
| COA  | Certificate of Analysis              | NY OCM requires one for every adult-use cannabis product |
...
| SKU  | Stock Keeping Unit                   | App Store Connect internal identifier                    |
