...
function getDefaultJoinedDate(joinedDate?: string | null) {
  return joinedDate || getCurrentIsoDate();
}
...
export function getLevelFromPoints(points: number): number {
  ...
  return Math.min(MAX_GAMIFICATION_LEVEL, Math.floor(Math.pow(points / 1000, 0.7)) + 50);
}
...
export function createDefaultGamificationState(
  ...
  return {
    ...
    badges: [],
    joinedDate: getDefaultJoinedDate(joinedDate),
    level,
    nextLevelPoints: getPointsForLevel(Math.min(level + 1, MAX_GAMIFICATION_LEVEL)),
    reviewsWithPhotos: 0,
    detailedReviews: 0,
    fiveStarReviews: 0,
    oneStarReviews: 0,
