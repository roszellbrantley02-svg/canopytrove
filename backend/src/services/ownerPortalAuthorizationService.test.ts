...
const originalModuleEntries = new Map(
  [
    firebaseModulePath,
    ownerPortalWorkspaceDataModulePath,
...
test(
  'getOwnerAuthorizationState falls back to ownerProfile.dispensaryId when no canonical storefront is linked yet',
  async () => {
        ...
        collection(name: string) {
          ...
          if (name === 'dispensaryClaims') {
            return {
              ...
              where() {
                throw new Error('Claim fallback query should not run when canonical claim id is resolved.');
              },
            };
          }
