...
As of the current repo state, the release check is flagging these concrete blockers:

- missing public `EXPO_PUBLIC_STOREFRONT_API_BASE_URL`
- missing public `EXPO_PUBLIC_SUPPORT_EMAIL`
- missing public privacy, terms, and community-guidelines URLs
- backend `CORS_ORIGIN` still set to `*`
- missing backend `EXPO_ACCESS_TOKEN`
- missing backend `ADMIN_API_KEY`
...
- Local block-author controls exist on storefront reviews.
...
1. Publish the legal documents and wire the public URLs into Expo env.
2. Configure the hosted backend and Stripe env set.
3. Fix backend `CORS_ORIGIN`, `EXPO_ACCESS_TOKEN`, and `ADMIN_API_KEY`.
4. Run `npm run release:check` until it passes.
5. Ship another `preview` build and finish phone QA.
6. Hide the owner demo from public release builds.
